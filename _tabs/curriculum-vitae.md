---
# the default layout is 'page'
icon: fas fa-info-circle
order: 1
---

# Nguyen Truong Giang
Nam Tu Liem, Ha Noi, Vietnam | [nguyentruonggiangptdq@gmail.com](mailto:nguyentruonggiangptdq@email.com) | 0335810127 | [linkedin.com/phongthien](https://www.linkedin.com/in/phongthien)

Experienced Senior Back-end Developer with a proven track record of delivering high-quality software solutions. Seeking a challenging role where I can leverage my expertise in Golang, Nodejs  to drive innovation and contribute to the success of a dynamic team.

## Education

- **VNU University of Engineering and Technology**                                            January 2017-December 2023
    
    *Bachelor of information technology*
    GPA: 3.23/4.0
    

## Work Experience

### Senior Backend Developer

*Thudo Multimedia,  Ha Noi, Viet Nam*                                                                           May 2021 - Present

- Participate in the development of OTT functions and solutions (SSAI, Livestream, VOD, etc.)
- Contribute to product development following the SaaS model
- Package products for distribution on platforms (Docker, Centos, Debian)
- Build a golang code base for the team to reduce coding effort
- Optimize code and test system performance
- Engage in the product deployment process to ensure stable system operation

### Fresher Backend Developer

*Anvita Company, Ha Noi, Viet Nam*                                                                              Mar 2020 - Jun  2021

- Participate in developing e-commerce functions
- Training new team members

### Internship Full-stack Developer

 *Mobio Vietnam JSC, Ha Noi, Viet Nam*                                                                             Mar 2019 - Jul  2019

- Learn frameworks (Angular, flask)
- Learn about code style and project architecture

## Skills

- Golang( echo, fiber, uber-fx, etc)
- Nodejs(Nestjs)
- K8s(helm, kustomer)
- Gitlab(ci/cd)
- Serverless (SAM)
- SQL(Postgress, Clickhouse)
- NoSQL(MongoDB)
- Rabitmq,Kafka,Redis,Minio
- HLS, DASH
- Cubejs
- Vuejs
- K6,Locust

## Certifications

- [AWS Certified Developer – Associate](https://www.credly.com/badges/4c579273-d80e-4512-9dcb-f16b22a52218/linked_in_profile)
    
    Amazon Web Service(AWS)
    
    Issued Feb 2024
    
- [AWS Certified Solutions Architect – Associate](https://www.credly.com/badges/58c16834-463e-4c9a-9688-4d7b35adee81/linked_in_profile)
    
    Amazon Web Service(AWS)
    
    Issued  Sep 2024
    

## References
